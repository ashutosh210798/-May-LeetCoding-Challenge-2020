struct TrieNode {
  ...
  TrieNode() {
      ...
      for (int i=0; i < 26; ++i) {
          Children[i] = NULL; // Fill the node with null pointers.
      }
  }
};
...
public:
    ...
    void insert(string word) {
        ...
        for (int i=0; i < length; ++i) {
            ...
            if (current->Children[num] == NULL) { // If it doesn't exist in the root, make it
    ...
    bool search(string word) {
        ...
        for (int i{}; i < length; ++i) {
            num = word[i] - 'a';
            if (current->Children[num] == NULL) { // If the letter doesn't exist, return false
                return false;
            } else {
